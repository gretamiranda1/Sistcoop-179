...
def _datos_grupal(codigo, huella, dni_1, dni_2):
    return {
        'importe_total': 40000,
        ...
        'personas': [
            {'dni': dni_1, 'nombre': 'Persona', 'apellido': 'Uno'},
            {'dni': dni_2, 'nombre': 'Persona', 'apellido': 'Dos'},
        ],
    }


def test_codigo_de_pago_individual_bloquea_un_pago_grupal(db, ejercicio, fondo_capital, datos_de_cuota):
    servicio_pagos.crear_pago_de_cuota(
        datos_de_cuota('30111111', 5000, 'OP-CRUZADO-1', 'huella-cruzada-1'))

    with pytest.raises(ErrorDeCarga) as excinfo:
        servicio_grupales.procesar_pago_grupal(
            _datos_grupal('OP-CRUZADO-1', 'huella-cruzada-2', '30222222', '30333333'))

    assert excinfo.value.errores == [MENSAJE_REPETIDO]


def test_codigo_de_pago_grupal_bloquea_un_pago_individual(db, ejercicio, fondo_capital, datos_de_cuota):
    servicio_grupales.procesar_pago_grupal(
        _datos_grupal('OP-CRUZADO-2', 'huella-cruzada-3', '30444444', '30555555'))

    with pytest.raises(ErrorDeCarga) as excinfo:
        servicio_pagos.crear_pago_de_cuota(
            datos_de_cuota('30666666', 5000, 'OP-CRUZADO-2', 'huella-cruzada-4'))
